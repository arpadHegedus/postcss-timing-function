        ...
        let parse = decl => {
            for (let [name, bezier] of Object.entries(transitions)) {
                if (decl.value.indexOf(name) >= 0) {
                    decl.value = decl.value.split(name).join(bezier);
                }
            }
        };
        css.walkDecls('animation', decl => { parse(decl); });
        css.walkDecls('transition', decl => { parse(decl); });
        css.walkDecls('animation-timing-function', decl => { parse(decl); });
        css.walkDecls('transition-timing-function', decl => { parse(decl); });
    }
});